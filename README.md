...
- No typing, no buzzers — teams shout their guess out loud. The Host taps
  **Team A got it** / **Team B got it** to award the point, or **Skip** if
  nobody can get it.
- Optional Letter Hints (Host reveals one letter at a time) and a
  per-puzzle timer (auto-skips with no score change when time runs out) —
  both off by default, configured on the setup screen.
...
- `js/game.js` — pure rules engine (deck, scoring, target score, optional
  timer and letter hints).
